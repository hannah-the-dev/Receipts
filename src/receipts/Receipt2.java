...
//		int k21_itemQty3 = 1;
		...
		System.out.printf("매장:%.26s\n", k21_address);							// 매장 주소 앞 26자리 출력
		System.out.printf("%.26s\n", k21_address.substring(26));				// 그 이후 자리 출력(최대 26자리)
		...
		gettingBytes by = new gettingBytes();									// gettingBytes 클래스 사용선언
		String item1Str = by.gettingSpaces(k21_item1, 14);						// 14번째 자리에서 자른 문자열1
		String item2Str = by.gettingSpaces(k21_item2, 14);						// 14번째 자리에서 자른 문자열2						
		String item3Str = by.gettingSpaces(k21_item3, 14);						// 14번째 자리에서 자른 문자열3
		...
		int k21_beforeTax = (int) Math.ceil(k21_amountSum/1.1);						// 과세금액 계산(소수점 올림)
